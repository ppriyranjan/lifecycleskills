...
function calculateDerivedMetrics(rawMetrics) {
  ...
  return {
    ...
    delivery_rate: safePercentage(delivered, recipients),
    bounce_rate: safePercentage(bounced, recipients),
    spam_rate: safePercentage(spam_complaints, recipients),

    // Engagement rates (unique activity)
    open_rate: safePercentage(opened, delivered),
    click_rate: safePercentage(clicked, delivered),
    click_to_open: safePercentage(clicked, opened),
    conversion_rate: safePercentage(converted, delivered),
    unsubscribe_rate: safePercentage(unsubscribed, delivered),

    // Revenue metrics
    revenue_per_recipient: recipients > 0 ? Math.round((revenue / recipients) * 100) / 100 : 0
  };
}
...
function calculateMetricsComparison(currentMetrics, previousMetrics, dateRanges) {
  ...
  const metricsToCompare = [
    ...
    'open_rate', 'click_rate', 'click_to_open',
